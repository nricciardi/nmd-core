...
impl TableBlock {
    ...
    fn html_standard_compile(&mut self, codex: &Codex, compilation_configuration: &CompilationConfiguration, compilation_configuration_overlay: CompilationConfigurationOverLay) -> Result<CompilationOutcome, CompilationError> {
        ...
        if let Some(ref id) = self.raw_id {
            ...
            html_table_attrs.push((String::from("id"), NUri::of_internal_from_without_sharp(&id, compilation_configuration_overlay.document_name().as_ref())?.build_without_internal_sharp()));
        }
...
impl Compilable for TableBlock {
...
impl ContentBlock for TableBlock {
...
mod test {
    ...
    fn load_table(nmd_text: &str, codex: &Codex) -> Box<dyn ContentBlock> {
        ...
        let rule = TableBlockLoadingRule::new();

        rule.load(nmd_text, &codex, LoadConfiguration::default()).unwrap()
