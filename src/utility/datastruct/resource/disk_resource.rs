...
impl DiskResource {
    pub fn new(location: PathBuf) -> Result<Self, MultiMediaObjectError> {

        Self::try_from(location)
    }

    pub fn create_parents_dir(&self) -> Result<(), MultiMediaObjectError> {
...
impl Resource for DiskResource {
    ...
    fn read(&self) -> Result<String, MultiMediaObjectError> {
        match fs::read_to_string(self.location.clone()) {           // TODO: remove clone
            ...
            Err(err) => Err(MultiMediaObjectError::ReadError(format!("error during read content of {}: {}", self.to_string(), err.to_string())))
        }
    }
