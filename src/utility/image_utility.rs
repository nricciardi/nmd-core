...
use crate::{base_parameter::output_format::OutputFormat, codex::{modifier::ModifiersBucket, Codex}, compilation::{compilation_configuration::{compilation_configuration_overlay::CompilationConfigurationOverLay, CompilationConfiguration}, compilation_error::CompilationError, compilation_outcome::CompilationOutcome}, mmo::{image::Image, nmd_unique_identifier::NmdUniqueIdentifier, source::Source, MultiMediaObjectError}};

use super::text_utility;


pub fn set_image_base64_embed_src(image: &mut Image, compression: bool) -> Result<(), MultiMediaObjectError> {
    ...
    if let Source::Bytes { mut bytes } = src {
        ...
        if compression {
            ...
            match optimized_png {
                ...
                Err(err) => return Err(MultiMediaObjectError::ElaborationError(format!("image compression error: {}", err)))
            }
...
pub fn compile_image_resource_in_html(image: &Image, img_classes: Vec<&str>, nuid: Option<&NmdUniqueIdentifier>, codex: &Codex, compilation_configuration: &CompilationConfiguration, compilation_configuration_overlay: CompilationConfigurationOverLay) -> Result<CompilationOutcome, CompilationError> {
