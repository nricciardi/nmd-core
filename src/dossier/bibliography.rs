...
impl Compilable for Bibliography {
    fn standard_compile(&mut self, format: &OutputFormat, codex: &Codex, compilation_configuration: &CompilationConfiguration, compilation_configuration_overlay: CompilationConfigurationOverLay) -> Result<CompilationOutcome, CompilationError> {
        ...
        match format {
            OutputFormat::Html => {
                let mut compilation_result = Text::new_empty();

                let mut compiled_title = TextPart::from(self.title.clone());

                compiled_title.compile(format, codex, compilation_configuration, compilation_configuration_overlay.clone())?;

                compilation_result.parts_mut().push(TextPart::new_fixed(String::from(r#"<section class="bibliography"><div class="bibliography-title">"#)));
                compilation_result.parts_mut().append(compiled_title.parts_mut());
                compilation_result.parts_mut().push(TextPart::new_fixed(String::from(r#"</div><ul class="bibliography-body">"#)));
        
                for (bib_key, bib_record) in self.content().iter() {
                    compilation_result.parts_mut().push(TextPart::new_fixed(format!(
                    ...
                    if let Some(authors) = bib_record.authors() {
                        ...
                        compilation_result.parts_mut().push(TextPart::new_fixed(format!(r#"<div class="bibliography-item-authors">{}</div>"#, authors.join(", "))));
                    }
        
                    if let Some(year) = bib_record.year() {

                        compilation_result.parts_mut().push(TextPart::new_fixed(format!(r#"<div class="bibliography-item-year">{}</div>"#, year)));
                    }
        
                    if let Some(url) = bib_record.url() {
        
                        compilation_result.parts_mut().push(TextPart::new_fixed(format!(r#"<div class="bibliography-item-url">{}</div>"#, url)));
                    }
        
                    if let Some(description) = bib_record.description() {

                        compilation_result.parts_mut().push(TextPart::new_fixed(format!(r#"<div class="bibliography-item-description">{}</div>"#, description)));
        
                    }
        
                    compilation_result.parts_mut().push(TextPart::new_fixed(String::from(r#"</div>"#)));
                }
        
                compilation_result.parts_mut().push(TextPart::new_fixed(String::from(r#"</ul></section>"#)));
