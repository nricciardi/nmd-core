...
use base64::Engine;
use url::Url;
use super::MultiMediaObjectError;
...
impl Source {
    ...
    pub fn try_to_base64(&self) -> Result<String, MultiMediaObjectError> {
    ...
    pub fn try_into_base64(self) -> Result<Self, MultiMediaObjectError> {

        Ok(Self::Base64String { base64: self.try_to_base64()? })

    }

    pub fn try_to_bytes(&self) -> Result<Vec<u8>, MultiMediaObjectError> {
        match self {
            Self::Remote { url } => return Err(MultiMediaObjectError::ElaborationError(format!("url '{}' cannot be parsed into bytes", url))),
            Self::Local { path } => {
    ...
    pub fn try_into_bytes(self) -> Result<Self, MultiMediaObjectError> {
