...
use crate::{dossier, mmo::{source::Source, uri::NUri, MultiMediaObjectError}};
...
impl Image {
    ...
    pub fn new(src: Source, mime_type: Option<String>, id: Option<NUri>, caption: Option<String>, style: Option<String>) -> Self {
    ...
    pub fn inferring_id_if_not_set(mut self, document_name: &impl ToString) -> Result<Self, MultiMediaObjectError> {

        if self.id.is_none() {
            
            if let Some(ref caption) = self.caption {

                self.id = Some(NUri::of_internal_from_without_sharp(caption, Some(document_name))?)

            } else {

                match &self.src {
                    Source::Remote { url } => self.id = Some(NUri::of_url(url.as_str())?),
                    Source::Local { path } => self.id = Some(NUri::of_asset(path.to_string_lossy().to_string().as_str())?),
    ...
    pub fn inferring_mime_type(mut self) -> Result<Self, MultiMediaObjectError> {
        ...
        match &self.src {
            ...
            Source::Local { path } => {
                ...
                if let Some(t) = mime_type {
                    ...
                    return Err(MultiMediaObjectError::InvalidResourceVerbose(format!("image {:?} mime type not found", self.src)));
                }
            },
...
impl FromStr for Image {
    type Err = MultiMediaObjectError;
