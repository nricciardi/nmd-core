...
pub struct CompilableString {

    #[getset(get = "pub", get_mut = "pub", set = "pub")]
    parts: Vec<CompilableStringPart>,
...
impl From<CompilableStringPart> for CompilableString {
    fn from(value: CompilableStringPart) -> Self {
        Self::new(vec![value])
    }
}

impl From<Vec<CompilableStringPart>> for CompilableString {
    fn from(value: Vec<CompilableStringPart>) -> Self {
        Self::new(value)
    }
}

impl Into<Vec<CompilableStringPart>> for CompilableString {
    fn into(self) -> Vec<CompilableStringPart> {
        self.parts  
    }
}

impl Into<String> for CompilableString {
    fn into(self) -> String {
        self.content()
    }
}

impl From<String> for CompilableString {
    fn from(value: String) -> Self {
        Self::from(CompilableStringPart::new_compilable(
            value,
...
impl From<&str> for CompilableString {
...
impl CompilableString {
    ...
    pub fn new(parts: Vec<CompilableStringPart>) -> Self {
    ...
    // pub fn new_with_nuid(parts: Vec<CompilableStringPart>, nuid: Option<NmdUniqueIdentifier>) -> Self {
    //     Self {
    //         parts,
    //         nuid
    //     }
    ...
    pub fn compilable_content_with_ends_positions(&self) -> (String, Vec<usize>) {
        ...
        self.parts.iter().for_each(|part| {
            match part {
                CompilableStringPart::Fixed { content: _ } => (),
                CompilableStringPart::Compilable { content, incompatible_modifiers: _ } => {
    ...
    pub fn parts_slice(&self, start: usize, end: usize) -> Result<Vec<CompilableStringPart>, CompilableStringError> {
    ...
    pub fn parts_slice_with_explicit_policy(&self, start: usize, end: usize, elaboration_policy: PartsSliceElaborationPolicy) -> Result<Vec<CompilableStringPart>, CompilableStringError> {

        let (compilable_content, ends) = self.compilable_content_with_ends_positions();

        if end > compilable_content.len() {
            return Err(CompilableStringError::ContentOverflow(compilable_content, start, end))
        }

        let mut parts_slice: Vec<CompilableStringPart> = Vec::new();
        ...
        let mut left_fixed_parts: Vec<&CompilableStringPart> = Vec::new();
        let mut right_fixed_parts: Vec<&CompilableStringPart> = Vec::new();
        ...
        while index < self.parts.len() {
            ...
            match part {
                CompilableStringPart::Fixed { content } => {
                    ...
                    match elaboration_position {
                        ...
                        ElaborationPosition::BeforeRange => {
                            if start_part_position_in_compilable_content <= start
                                ...
                                let part = CompilableStringPart::new(
...
impl CompilableString {
    ...
    fn compile_with_compilation_rule(&mut self, (rule_identifier, rule): (&ModifierIdentifier, &Box<dyn CompilationRule>), format: &OutputFormat, compilation_configuration: &CompilationConfiguration, compilation_configuration_overlay: CompilationConfigurationOverLay) -> Result<(), CompilationError> {
        ...
        let mut compiled_parts: Vec<CompilableStringPart> = Vec::new();     // final output
        ...
        while parts_index < parts.len() {      // there are other parts
            ...
            let mut matched_parts: Vec<CompilableStringPart> = Vec::new();
                            ...
                            if !match_found && part_end_position_in_compilable_content <= match_start {      // there is no match in this part
                                ...
                                if match_end <= part_end_position_in_compilable_content {       // matching end is in this part
                                    ...
                                    compiled_parts.append(
                                        &mut rule.compile(
                                            &CompilableString::from(matched_parts),
                                            format,
                                            compilation_configuration,
...
impl Compilable for CompilableString {
